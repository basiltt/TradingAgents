...
export const StrategyPnLView = memo(function StrategyPnLView({ rows, loading }: StrategyPnLViewProps) {
  if (loading) {
    return <div className="text-sm text-zinc-500 py-6 text-center" data-testid="strategy-pnl-loading">Loading strategy breakdown…</div>;
  }
  if (!rows || rows.length === 0) {
    return (
      <div className="text-sm text-zinc-500 py-6 text-center" data-testid="strategy-pnl-empty">
  ...
  return (
    <table className="w-full text-sm" data-testid="strategy-pnl-table">
      ...
      <tbody>
        {rows.map((r) => (
          <tr key={`${r.strategy_kind}:${r.direction}`} className="border-t border-zinc-800/60" data-testid="strategy-pnl-row">
            ...
            <td className={`py-2 pr-4 text-right tabular-nums font-medium ${r.total_pnl >= 0 ? "text-emerald-500" : "text-red-500"}`}>
              {fmtPnl(r.total_pnl)}
            </td>
            <td className={`py-2 pr-4 text-right tabular-nums ${r.avg_pnl >= 0 ? "text-emerald-500" : "text-red-500"}`}>
              {fmtPnl(r.avg_pnl)}
            </td>
            <td className="py-2 text-right tabular-nums text-zinc-400">{fmtHold(r.avg_hold_minutes)}</td>
          </tr>
        ))}
      </tbody>
    </table>
