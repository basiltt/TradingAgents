...
export const TradeRow = memo(function TradeRow({
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3 text-[11px] font-mono tabular-nums"><PnLDisplay value={trade.realized_pnl ?? (isActive ? 0 : null)} /></td>
      <td className="px-4 py-3 text-[11px] font-mono tabular-nums"><PnLDisplay value={trade.unrealized_pnl ?? (isActive ? 0 : null)} /></td>
      <td className="px-4 py-3 text-[11px] font-mono tabular-nums text-muted-foreground/60">{formatPrice(trade.fees ?? 0)}</td>
          ...
          <div className="opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
            {trade.status === "pending" ? (
              <Button
                variant="ghost"
                size="sm"
                className="h-6 text-[10px] px-2 rounded-md text-amber-400 hover:text-amber-300 hover:bg-amber-500/10 cursor-pointer"
                disabled={isPending}
              ...
              <Button
                variant="ghost"
                size="sm"
                className="h-6 text-[10px] px-2 rounded-md text-red-400 hover:text-red-300 hover:bg-red-500/10 cursor-pointer"
                disabled={isPending}
