  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        shortLabel: "Backtest",
        to: "/backtest",
        description: "Simulate the auto-trade cycle over historical scans with full metrics.",
        icon: FlaskConical,
        keywords: ["backtest", "simulation", "historical", "strategy testing", "equity curve"],
        matches: (pathname) => pathname.startsWith("/backtest"),
      },
      {
        id: "trades",
        label: "Trades",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        shortLabel: "MCP",
        to: "/mcp",
        description: "Expose the app to an external AI agent with a context-aware tool budget and human approval.",
        icon: Network,
        keywords: ["mcp", "model context protocol", "ai agent", "integration", "tools", "automation"],
        matches: (pathname) => pathname.startsWith("/mcp"),
      },
    ],
  },
];
...
export const mobileDockItems = navItems.filter((item) =>
  ["home", "scanner", "accounts", "analytics"].includes(item.id),
);
