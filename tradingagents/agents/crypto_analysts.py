...
def create_crypto_portfolio_manager(llm, max_leverage: int = 20):
    ...
    def node(state):
        ...
        prompt = (
            ...
            f"- **Overweight**: Moderately bullish, go long with moderate conviction\n"
            f"- **Hold**: No compelling trade — evidence is mixed or insufficient (REJECT = Hold)\n"
            f"- **Underweight**: Moderately bearish, go short with moderate conviction\n"
            f"- **Sell**: Strong conviction to go SHORT — price will fall significantly\n\n"
            f"This system OPENS new positions (Long or Short). It does NOT manage existing holdings.\n"
            f"If you REJECT the trade, your rating MUST be Hold.\n"
            f"If you APPROVE/MODIFY a Long trade, use Buy (strong) or Overweight (moderate).\n"
            f"If you APPROVE/MODIFY a Short trade, use Sell (strong) or Underweight (moderate).\n\n"
