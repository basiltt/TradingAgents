"""Content-preservation for the fundamentals_analyst prompt refactor."""
import re
...
def _render_fundamentals_analyst(date="2026-06-06", symbol="BTCUSDT",
    ...
    with patch("tradingagents.agents.analysts.fundamentals_analyst.get_language_instruction",
               return_value=lang), \
...
class TestFundamentalsAnalystContentPreserved:
    def test_all_content_reaches_model(self):
        msgs = _render_fundamentals_analyst()
        joined = " ".join(_norm(_msg_text(m)) for m in msgs)
        for fragment in ["researcher tasked", "2026-06-06", "BTCUSDT futures",
    ...
    def test_system_prefix_byte_stable_across_date_and_symbol(self):
        ...
        def _sys_for(date, symbol, instrument):
            msgs = _render_fundamentals_analyst(date=date, symbol=symbol,
                                                instrument=instrument, lang="")
