...
def test_nfr004_preset_budgets_within_ceilings():
    ...
    # `backtest_run` legitimately exposes the FULL backtest+AutoTradeConfig surface
    # — which the Regime Multi-Strategy merge expanded by ~30 mr_*/regime_*/btc_vol_*
    # /session_* params — so it carries a larger (but intentional) schema. The real
    # budget guard is the per-preset ceilings above (all pass: backtest_only 5444/8000);
    # this per-tool check only catches a runaway/accidental schema. Allow the known
    # large backtest tool a higher bound; everything else stays under the tight cap.
    _LARGE_TOOL_BOUND = {"backtest_run": 2_500}
    for s in specs:
        est = estimate_tool_tokens(s)
        bound = _LARGE_TOOL_BOUND.get(s.name, PER_TOOL_SOFT_CAP * 3)
        assert est <= bound, f"{s.name} schema {est} tok is implausibly large (bound {bound})"
