...
def test_migration_versions_sequential_no_dupes():
    ...
    assert versions[-1] == 48, "latest migration should be 48"
...
def test_check_enum_matches_pydantic_literal_cohort():
    ...
    literal_vals = set(getattr(fields["strategy_cohort"].annotation, "__args__", ()))
    assert db_vals == {"trend", "mean_reversion"}
    assert db_vals == literal_vals, f"DB {db_vals} != Pydantic {literal_vals}"
