...
| P6 EVAL GATE | ATTEMPTED — INCONCLUSIVE | Harness built (dfa9811) + run vs real Anthropic. Limitation found: market_analyst tool-less rarely emits the FINAL TRANSACTION PROPOSAL line (~50% None), so parity is unmeasurable. Parser hardened to strict (removed a last-token fallback that misread "not a sell signal" as SELL). Branch ships on STATIC evidence (default-OFF + byte-for-byte preservation). Harness needs redesign before any default-ON. See cache-parity-eval-results.md. |
