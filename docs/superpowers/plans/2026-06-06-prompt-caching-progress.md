...
| P6 EVAL GATE | DONE — PASS | Harness built (dfa9811), run, and PASSED. Two harness bugs found+fixed: (1) decision-extraction — market_analyst tool-less rarely reached the proposal line (~50% None); fixed with a decision-first task template (proposal line FIRST, identical for OLD+NEW). (2) asymmetric sampling — OLD denoised over K=5 but NEW sampled once, comparing OLD-mode vs NEW-single-draw → spurious FAIL on 1 noisy fixture (SOL 2026-02-11). Disambiguation probe proved it was sampling noise (NEW modes SELL 7/7); fixed by sampling BOTH arms K=5 and comparing mode-vs-mode (budget N*K*2=300). **Definitive run (2026-06-06, 300 calls, FREE via localhost:4141 proxy claude-sonnet-4.6/dummy key): agreement=1.000 (>=0.993), McNemar p=1.0000, noise=0.007 → PASS.** 38 harness tests green (+3 symmetric regressions). The P3 role-move is now empirically validated, not just static-evidence. See cache-parity-eval-results.md. |
