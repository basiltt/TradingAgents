...
def _get_service(request: Request):
    """Retrieve SignalAnalyticsService from app state or raise 503."""
    svc = getattr(request.app.state, "signal_analytics_service", None)
    if svc is None:
...
async def get_current_regimes(request: Request):
